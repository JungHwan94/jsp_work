...
<body>
	...
	> 비동기(ajax)
	<ul>
		<li>현재 페이지를 그대로 유지하면서 중간중간 추가적으로 필요한 요청을 보내줄 수 있음</li>
		<li>요청을 보냈다고 해서 다른 페이지로 넘어가지 않음(현 페이지 그대로 유지)</li>
		<li>요청을 보내놓고 그에 해당하는 응답(데이터)이 돌아올때까지 현재페이지에서 다른 작업</li>
		<li>페이지가 깜빡이지 않음</li>
	</ul>
		...
		* 비동기식의 단점<br>
		...
		- 에러 발생시 디버깅 어려움<br>
		- 요청 처리후에 돌아온 응답데이터를 가지고 현재페이지에 새로운 요소를 만들어서 보여줘야 함
			=> dom요소들을 새롭게 만들어내는 구문을 알아야됨
		<br><br>
	...
	* 주요속성
	...
	- error: ajax 통신이 실패했을때 실행할 함수 정의
   ...
   - timeout : 서버 요청 시 응답 대기 시간(milisecond)
	...
	<h3>1. 버튼 클릭 get방식으로 서버에 요청 및 응답</h3>
	입력: <input type="text" id="input1">&emsp;<input type="button" id="btn1">
	<br>
				...
				if($idInput.val().length >= 3) {
					$.ajax({
						...
						success: function(result) {
							if(result == "idN") {
								$('#checkResult').show();
								$('#checkResult').css("color", "red").text("이미 사용중인 아이디");
								$('#enrollFrom :submit').attr("disabled", true);
							} else {
								$('#checkResult').show();
								$('#checkResult').css("color", "green").text('halo');
	...
	<h3>3. 서버에 데이터 전송한 후, 조회된 bean객체를 응답데이터로 받을때</h3>
