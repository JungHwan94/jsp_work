...
<body>
	...
	* 변수 선언과 동시에 초기화
		...
		> c:set으로 선언된 변수는 EL구문으로만 접근가능(스크림팀원소로 접근 불가)	
	</pre>
	...
	<%-- num1 변수 값: <%=num1 %><br> 사용불가 --%>
	
	<%-- <c:set var="result" value="num1+num2" scope="session" /> 문자열을 변수에 넣음--%>
	...
	* 변수 삭제 (c:remove var="제거하고자하는 변수명" [scope=""])
	...
	- scope를 저장하지 않으면 모든 scope에서 해당 변수를 모두 다 찾아서 제거함
	</pre>
	삭제전 result: ${result}<br><br>
	
	1) 특징 scope(request) 지정하여 삭제<br>
	...
	<h4>1.3 변수 출력</h4>
	<pre>
	* 변수 출력 (c:out value="출력하고자하는 값" [default="기본값"] [escapeXml="true|false"])
	...
	<c:out value="${outTest}" /><br><!-- escapeXml생략시 기본값 true => 문자열로 인식 -->
	...
	* 향상된 for문
	...
	<%
		...
		list.add(new Person("가나다", 29, "여자"));
		list.add(new Person("마바사", 25, "남자"));
	%>
	...
	* 구분자를 통해서 분리된 각각의 문자열에 순차적으로 접근하여 반복 수행
	...
	<a href="list.do?Page=1&num=2">기존 방식</a>
